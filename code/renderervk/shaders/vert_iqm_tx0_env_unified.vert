// IQM GPU skinning unified vertex shader (single texture env, color path)
...
vec4 BuildGpuMd3Color(vec3 position, vec3 normal, vec4 fallbackColor)
{
    ...
    if (useGpuSolidRgba) { color.rgb = ubo.lightPos.xyz; color.a = ubo.lightColor.w; }
    else if (useGpuDiffuseRgb) { const float incoming = max(dot(normal, ubo.lightVector.xyz), 0.0); color = vec4(clamp(ubo.lightPos.xyz + incoming * ubo.lightColor.xyz, 0.0, 1.0), 1.0); }
    else if (useGpuExactVertexRgb) color.rgb = fallbackColor.rgb;
    else if (useGpuVertexRgb) color.rgb = fallbackColor.rgb * pc.md3Anim.z;
    else if (useGpuOneMinusVertexRgb) color.rgb = (vec3(1.0) - fallbackColor.rgb) * pc.md3Anim.z;
    if (useGpuSpecularAlpha) color.a = CalcGpuSpecularAlpha(position, normal);
    else if (useGpuPortalAlpha) color.a = clamp(length(ubo.eyePos.xyz - position) * ubo.fogEyeT.z, 0.0, 1.0);
    else if (useGpuOneMinusVertexAlpha) color.a = 1.0 - fallbackColor.a;
    else if (useGpuVertexAlpha) color.a = fallbackColor.a;
    else if (useGpuUniformAlpha) color.a = ubo.lightColor.w;
...
void main()
{
    ...
    if (ubo.eyePos.w > 0.5)
        frag_tex_coord0 = ApplyGpuTcMods(position, (ubo.lightPos.w > 0.5) ? calc_env_tc_fpscr(position, normal) : calc_env_tc_fp(position, normal));
    else
