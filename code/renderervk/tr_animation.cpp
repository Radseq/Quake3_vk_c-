...
bool R_PrepareMDRSurfaces(trRefEntity_t& ent, entityFrameCache_t& cache)
{
	...
	if (R_MDRCullModel(header, ent) == CULL_OUT)
	{
		return false;
	}
	...
	cache.actorLod = lodnum;
	cache.actorFogNum = R_MDRComputeFogNum(header, ent);
...
void R_MDRAddAnimSurfaces(trRefEntity_t& ent)
{
	...
	for (i = 0; i < lodnum; i++)
	{
		lod = reinterpret_cast<mdrLOD_t*>((byte*)lod + lod->ofsEnd);
	}

	surface = reinterpret_cast<mdrSurface_t*>((byte*)lod + lod->ofsSurfaces);
	...
	for (i = 0; i < lod->numSurfaces; i++)
	{
		...
		surface = reinterpret_cast<mdrSurface_t*>((byte*)surface + surface->ofsEnd);
	}
}
