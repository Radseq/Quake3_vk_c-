...
static int SetLightmapParams(int numLightmaps, const int maxTextureSize)
{
	...
	{
		lightmapWidth = log2pad_plus(lightmapWidth + LIGHTMAP_LEN, 1);
		lightmapCountX = lightmapWidth / LIGHTMAP_LEN;
		if (lightmapCountX * lightmapCountY >= numLightmaps)
			break;
		lightmapHeight = log2pad_plus(lightmapHeight + LIGHTMAP_LEN, 1);
...
static void R_LoadMergedLightmaps(const lump_t &l, byte *image)
{
	...
	tr.lightmaps = reinterpret_cast<image_t **>(ri.Hunk_Alloc(tr.numLightmaps * sizeof(image_t *), h_low));
...
{
	if (Q_stricmp_cpp(std::string_view(s_worldData.baseName.data(), s_worldData.baseName.size()), "mapel4b") == 0 && count == 86)
	{
...
void RE_LoadWorldMap(const char *name)
{
	...
	Q_strncpyz_cpp(s_worldData.name, name, sizeof(s_worldData.name));

	Q_strncpyz_cpp(s_worldData.baseName, COM_SkipPath(s_worldData.name.data()), sizeof(s_worldData.name));
	COM_StripExtension_cpp(std::string_view(s_worldData.baseName.data(), s_worldData.baseName.size()), s_worldData.baseName);
