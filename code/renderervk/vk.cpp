...
static void init_vulkan_library(void)
{
	...
	// Pick GPU and create vk_inst.device + vk_inst.physical_device + vk_inst.queue_family_index,
	// also sets vk_inst.base_format/present_format and attachment formats.
	if (!pick_and_create_device(
		vk_inst,
		vk_instance,
		vk_surface,
		r_device->integer,
		/*verbose*/ true))
	{
		// pick_and_create_device calls ri.Error(ERR_FATAL) on failure
...
static void vk_create_shader_modules(void)
{
	...
	vk_inst.modules.vert.md3_blend[0][0] = SHADER_MODULE(vert_md3_blend_unified_vert_spv);
	vk_inst.modules.vert.md3_blend[0][1] = SHADER_MODULE(vert_md3_blend_unified_vert_spv);
	vk_inst.modules.vert.md3_blend[1][0] = SHADER_MODULE(vert_md3_blend_env_unified_vert_spv);
	vk_inst.modules.vert.md3_blend[1][1] = SHADER_MODULE(vert_md3_blend_env_unified_vert_spv);
...
static void reset_vk_instance(Vk_Instance& s) noexcept
{
	...
	for (auto& row : s.modules.vert.md3_blend)
		for (auto& m : row)
			reset_to_default(m);
...
void vk_shutdown(const refShutdownCode_t code)
{
	...
	for (auto& row : vk_inst.modules.vert.md3_blend)
	{
		for (auto& m : row)
		{
			if (m)
			{
				vk_inst.device.destroyShaderModule(m);
				m = nullptr;
			}
		}
	}
...
static vk::Buffer shade_bufs[10];
...
{
	const shaderStage_t* stage = VK_GpuMd3CurrentStage();
	return stage ? R_GpuMd3SecondaryColorMode(*stage, 0u) : 0u;
}

static ID_INLINE uint32_t VK_GpuMd3CurrentSecondaryColorMode(const uint32_t bundleIndex) noexcept
{
	const shaderStage_t* stage = VK_GpuMd3CurrentStage();
	return stage ? R_GpuMd3SecondaryColorMode(*stage, bundleIndex) : 0u;
}

static ID_INLINE bool VK_GpuMd3UsesRawVertexColor() noexcept
{
	return R_GpuMd3ColorModeUsesRawVertexColor(VK_GpuMd3CurrentColorMode());
}

static ID_INLINE bool VK_GpuMd3CurrentSecondaryUsesRawVertexColor(const uint32_t bundleIndex) noexcept
{
	return R_GpuMd3ColorModeUsesRawVertexColor(VK_GpuMd3CurrentSecondaryColorMode(bundleIndex));
}
...
void vk_bind_geometry(const uint32_t flags)
{
	if (tess.gpuMd3Active)
	{
		...
		if ((flags & TESS_ST1) && (flags & (TESS_RGBA1 | TESS_RGBA2)))
		{
			...
			const uint32_t colorMode1 = VK_GpuMd3CurrentSecondaryColorMode(1u);
			const uint32_t colorMode2 = VK_GpuMd3CurrentSecondaryColorMode(2u);
			const bool rawColor1 = VK_GpuMd3CurrentSecondaryUsesRawVertexColor(1u);
			const bool rawColor2 = VK_GpuMd3CurrentSecondaryUsesRawVertexColor(2u);
			...
			shade_bufs[8] = vk_inst.cmd->vertex_buffer;
			shade_bufs[9] = vk_inst.cmd->vertex_buffer;
