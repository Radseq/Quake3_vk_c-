                            ...
                            orientationr_t *orient)
{
    ...
    myGlMultMatrix_plus(glMatrix, viewParms->world.modelMatrix, orient->modelMatrix);

    // calculate the viewer origin in the model's space
    // needed for fog, specular, and environment mapping
    VectorSubtract(viewParms->ort.origin, orient->origin, delta);
...
static void R_RotateForViewer(void)
{
    ...
    myGlMultMatrix_plus(viewerMatrix, s_flipMatrix_plus, tr.ort.modelMatrix);
...
static void R_GetModelViewBounds(int *mins, int *maxs)
{
    ...
    // premultiply
    myGlMultMatrix_plus(tr.ort.modelMatrix, tr.viewParms.projectionMatrix, mvp);
...
static void R_PlaneForSurface(const surfaceType_t *surfType, cplane_t *plane)
{
    ...
    case SF_TRIANGLES:
        ...
        PlaneFromPoints_plus(plane4, v1->xyz, v2->xyz, v3->xyz);
        VectorCopy(plane4, plane->normal);
        plane->dist = plane4[3];
        return;
    case SF_POLY:
        poly = (srfPoly_t *)surfType;
        PlaneFromPoints_plus(plane4, poly->verts[0].xyz, poly->verts[1].xyz, poly->verts[2].xyz);
        VectorCopy(plane4, plane->normal);
        plane->dist = plane4[3];
        return;
    default:
...
static bool IsMirror(const drawSurf_t *drawSurf, int entityNum)
{
    ...
    if (entityNum != REFENTITYNUM_WORLD)
    {
        ...
        R_RotateForEntity_plus(tr.currentEntity, &tr.viewParms, &tr.ort);
...
//         R_TransformModelToClip_plus(tess.xyz[i], tr.ort.modelMatrix, tr.viewParms.projectionMatrix, eye, clip);
                                    ...
                                    vec3_t pvsOrigin, portalView_t *portalView)
{
    ...
    PerpendicularVector_plus(surface->axis[1], surface->axis[0]);
    ...
    for (i = 0; i < tr.refdef.num_entities; i++)
    {
        ...
        AxisCopy_plus(e->e.axis, camera->axis);
        ...
        if (e->e.oldframe)
        {
            ...
            if (e->e.frame)
            {
                ...
                RotatePointAroundVector_plus(camera->axis[1], camera->axis[0], transformed, d);
...
//     {
//         R_DecomposeSort_plus((drawSurfs + i)->sort, &entityNum, &shader, &fogNum, &dlighted);
...
//     R_SetupProjection_plus(&tr.viewParms, r_zproj->value, true);
