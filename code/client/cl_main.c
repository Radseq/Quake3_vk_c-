...
static void CL_CompleteRecordName(const char *args, int argNum )
{
	if ( argNum == 2 )
	{
		...
		Com_sprintf( demoExt, sizeof( demoExt ), "%d", DEMOEXT, com_protocol->integer );
...
static void CL_CompleteDemoName(const char *args, int argNum )
{
	if ( argNum == 2 )
	{
		...
		Field_CompleteFilename("demos", "." DEMOEXT "??", false, FS_MATCH_ANY | FS_MATCH_STICK | FS_MATCH_SUBDIRS);
